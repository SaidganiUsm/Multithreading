...
public class LinkFetcherRecursive extends RecursiveAction {
    private LinkFetcher node;

    public LinkFetcherRecursive(LinkFetcher node) {
        this.node = node;
    }
    ...
    protected void compute() {
        try {
            ...
            for (Element a : elements) {
                ...
                if (isCorrectUrl(childUrl)) {
                    ...
                    node.addChild(new LinkFetcher(childUrl));
                }
            }
        ...
        for (LinkFetcher child : node.getChildren()) {
            LinkFetcherRecursive task = new LinkFetcherRecursive(child);
            task.compute();
        }
    }
